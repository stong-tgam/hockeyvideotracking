...
def main(video_path, model_path, output_path=None):
    """
    Main function to run the hockey analytics pipeline
    """
    print("Initializing Hockey Analytics System...")
    ...
    print("Starting analysis... Press 'q' to quit, 's' to select player for heatmap")
    ...
    while cap.isOpened():
        ...
        # Display frame
        cv2.imshow('Hockey Analytics - Original + Rink View', combined_frame)
        ...
        if key == ord('q'):
            ...
            if tracker.jersey_numbers:
                ...
                try:
                    jersey_input = input("Enter jersey number to show heatmap (or 'none' to clear): ")
                    if jersey_input.lower() == 'none':
                        selected_player_for_heatmap = None
                        print("Heatmap cleared")
                    else:
                        # Find the tracker ID for this jersey number
                        for tid, jnum in tracker.jersey_numbers.items():
                            if str(jnum) == jersey_input:
                                selected_player_for_heatmap = tid
                                print(f"Showing heatmap for player #{jnum}")
                                break
                        else:
    ...
    for tracker_id, distance in analytics.player_distances.items():
        jersey_num = tracker.get_jersey_number(tracker_id)
    ...
    if analytics.puck_control_history:
        ...
        for player_id, data in analytics.puck_control_history.items():
            jersey_num = tracker.get_jersey_number(player_id)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Hockey Analytics System')
